...
static void lvgl_touch_cb(lv_indev_t * indev, lv_indev_data_t *indevData) {
    uint16_t      x        = 0x00;
    uint16_t      y        = 0x00;
    LcdTouchPanel DevTouch = Custom_GetLcdTouchPanel();
    if (DevTouch.GetCoords(&x, &y)) {
        // The controller returns a 12 bit value, so a bad read can be far
        // larger than the panel. Subtracting it then gives a negative
        // coordinate, which the old upper-bound test could not catch.
        int32_t px = (int32_t) LCD_H_RES - 1 - (int32_t) x;
        int32_t py = (int32_t) LCD_V_RES - 1 - (int32_t) y;
        indevData->point.x = px < 0 ? 0 : (px > LCD_H_RES - 1 ? LCD_H_RES - 1 : px);
        indevData->point.y = py < 0 ? 0 : (py > LCD_V_RES - 1 ? LCD_V_RES - 1 : py);

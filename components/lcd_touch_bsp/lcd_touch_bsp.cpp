...
uint8_t LcdTouchPanel::GetCoords(uint16_t *x, uint16_t *y) {
    ...
    if (GestureNum[0] && (Event != 0x01)) {
        // A failed read leaves Gpos all zero, which used to be reported as a
        // real press at the origin: a phantom tap on every I2C hiccup.
        if (i2cbus_.i2c_read_buff(touch_dev_handle, 0x03, Gpos, 4) != ESP_OK) return 0;
